...
function Setup() {
  ...
  const onSubmit = async (values: SetupValues) => {
    setSubmitError(null);
    const githubTrimmed = values.githubUsername.trim();
    if (githubTrimmed && githubTrimmed === debouncedGithub && !githubQuery.isFetching && github?.status === 'not_found') {
      setSubmitError(github.summary || "We couldn't find that GitHub account. Fix the username or clear the field to continue.");
      return;
    }
