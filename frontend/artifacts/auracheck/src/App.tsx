...
type Scorecard = { overallScore: number; summary: string; dimensions: { label: string; score: number; note: string }[]; evidence: string[] };
...
const demoScorecard: Scorecard = {
  overallScore: 78,
  summary: 'You bring considered thinking and credible technical range. The next lift is making your impact easier to feel in the first 30 seconds.',
  dimensions: [
    { label: 'Clarity', score: 82, note: 'Strong structure. Lead with the outcome before the process.' },
    { label: 'Technical depth', score: 76, note: 'Good fundamentals with room for sharper trade-off language.' },
    { label: 'Confidence', score: 73, note: 'Your ideas land; let your pauses work harder for you.' },
    { label: 'Evidence', score: 84, note: 'Your examples were specific and supported by your footprint.' },
  ],
  evidence: ['Shipped a real-time collaboration feature used by 1,200+ students.', 'Five public repositories reinforce your JavaScript and Python fluency.', 'Explained a complex API decision with a clean, user-first rationale.'],
};
...
function Setup() {
  ...
  const [resumeStatus, setResumeStatus] = useState<'idle' | 'uploading' | 'done'>('idle');
  ...
  const githubQuery = useGetGithubFootprint(watchedGithub || 'auracheck-demo', { query: { enabled: Boolean(watchedGithub), queryKey: getGetGithubFootprintQueryKey(watchedGithub || 'auracheck-demo') } });
  ...
  const onSubmit = (values: SetupValues) => {
    ...
    startSession.mutate({ data: payload }, {
      onSuccess: (session) => { saveSession(session as Session); setLocation('/interview'); },
      onError: () => { saveSession({ ...demoSession, openingPrompt: `Welcome, ${values.candidateName.split(' ')[0]}. Let’s start with the work you’re most proud of — what made it matter?` }); setDemoNotice(true); setTimeout(() => setLocation('/interview'), 900); },
    });
  };
  const onResume = (event: ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0];
    if (!file) return;
    setResumeName(file.name); form.setValue('resumeName', file.name); setResumeStatus('uploading');
    uploadResume.mutate({ data: { fileName: file.name } }, { onSettled: () => setResumeStatus('done') });
  };
  return (
    <div className="page-enter mx-auto max-w-[1080px] px-5 py-8 sm:px-8 sm:py-12"><div className="mb-10 max-w-[640px]"><SectionLabel>New practice room</SectionLabel><h1 className="font-display text-[clamp(42px,6vw,70px)] leading-[.95] tracking-[-.045em]">Bring your whole<br /><em className="text-[#8d67ae]">signal</em> into the room.</h1><p className="mt-5 max-w-[510px] text-[14px] leading-7 text-[#77727d]">Give the panel enough context to ask better questions. Nothing here needs to be perfect — it just needs to be yours.</p></div>
      ...
      </form></Form>{demoNotice && <div className="fixed bottom-5 right-5 z-30 flex items-center gap-3 rounded-xl border border-[#dbc8e7] bg-[#f8f1fb] px-4 py-3 text-[12px] text-[#735786] shadow-xl" data-testid="status-demo-mode"><Sparkles size={15} /> Demo room opened while the panel warms up.</div>}</div>
  );
}
...
function Interview() {
  ...
  const end = () => {
    setEnding(true);
    endSession.mutate({ sessionId: session.sessionId }, { onSuccess: (scorecard) => { saveScorecard(scorecard as Scorecard); setLocation('/scorecard'); }, onError: () => { saveScorecard(demoScorecard); setTimeout(() => setLocation('/scorecard'), 500); } });
  };
